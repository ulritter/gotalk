...
func handleServerSession(clientInputChannel <-chan ClientInput, nl Newline) {
	...
	for input := range clientInputChannel {
		...
		case *UserJoinedEvent:
			...
			input.user.session.WriteStatus(fmt.Sprintf(lang.Lookup(actualLocale, "Welcome")+" %s", input.user.name))
			for _, user := range room.users {
				if user != input.user {
					user.session.WriteStatus(" ")
					user.session.WriteStatus(fmt.Sprintf("%s ", input.user.name) + lang.Lookup(actualLocale, "entered the room"))
				}
			}
		case *UserLeftEvent:
			...
			for _, user := range room.users {
				if user != input.user {
					user.session.WriteStatus(" ")
					user.session.WriteStatus(fmt.Sprintf("%s "+lang.Lookup(actualLocale, "left the room"), input.user.name))
		...
		case *UserChangedNickEvent:
			...
			for _, user := range room.users {
				if user != input.user {
					user.session.WriteStatus(" ")
					user.session.WriteStatus(fmt.Sprintf("[%s] "+lang.Lookup(actualLocale, "has changed his nick to:")+" [%s]", event.user.name, event.nick))

				} else {
					user.session.WriteStatus(" ")
					user.session.WriteStatus(lang.Lookup(actualLocale, "You have changed your nick"))
					user.session.WriteStatus(fmt.Sprintf(lang.Lookup(actualLocale, "from")+" [%s] "+lang.Lookup(actualLocale, "to")+" [%s]", event.user.name, event.nick))
				}
			}
		...
		case *ListUsersEvent:
			...
			input.user.session.WriteStatus(lang.Lookup(actualLocale, "User list:"))

			for _, user := range room.users {
				input.user.session.WriteStatus(fmt.Sprintf("[%s] - "+lang.Lookup(actualLocale, "joined at")+" [%s]", user.name, user.timejoined))
			}
		}
	}
}
